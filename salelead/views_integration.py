...
def ingest_opportunity(request, source_system):
    ...
    # 2️⃣ META webhook verification (GET only)
    if request.method == "GET":
        if source_system != "META":
            return Response(
                {"detail": "GET not supported for this source"},
                status=status.HTTP_405_METHOD_NOT_ALLOWED,
            )
        ...
        return Response(
            {"detail": "Meta verification failed"},
            status=status.HTTP_403_FORBIDDEN,
        )

    # 3️⃣ POST → Lead ingestion
    normalizer = NORMALIZERS.get(source_system)
    if not normalizer:
        return Response(
            {"detail": f"No normalizer defined for '{source_system}'"},
